...
if (titleEl) {
  const texts = ['est. <span class="red">2026</span>', 'E-Sports <span class="red">Design</span>', 'Business <span class="red">Branding</span>', 'Academic <span class="red">Design</span>'];
  let i = 0;
  setInterval(() => {
    i = (i + 1) % texts.length;
...
function overlayPlay() {
  ...
  bgMusic.play().then(() => {
    ...
    document.getElementById('music-label').textContent = '♪ Hold me down';
  }).catch(() => {});
}
...
function toggleMusic() {
  ...
  if (musicPlaying) {
    ...
    label.textContent = '♪ Hold me down';
    musicPlaying = true;
  }
}
